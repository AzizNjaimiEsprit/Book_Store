...
public class userService implements IServiceUser<User> {
    ...
    public void AddAdmin(User t) {
        ...
        String req = "Insert into USER ( id,full_name,email,telephone,login,password)  values (?,?,?,?,?,?)";
        try {
            ...
            pst.setString(6, BCrypt.hashpw(t.getPassword(), BCrypt.gensalt(12)));
            String verifcode = BCrypt.hashpw(t.getFullName(), BCrypt.gensalt(12));
            userService us=new userService();
            us.mailling(t.getEmail(), "Your verification code is : " + verifcode);
            pst.executeUpdate();
    ...
    public User GetUser(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                u = new User (rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(7),rs.getString(8));
            }
        }
        catch (SQLException ex) {
                System.out.println(ex.getMessage());
